...
export default async function StudioHubPage({ params }: Props) {
  ...
  const entitlement = await requireEntitlement(ctx.currentWorkspace.id, "studio");

  const hub = HUBS.find((item) => item.slug === "studio");
  if (!hub) {
    return <div className="text-sm text-muted-foreground">Hub not found.</div>;
  }
