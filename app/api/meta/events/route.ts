...
export async function POST(req: NextRequest) {
  ...
  const { workspaceId: rawWorkspaceId, wabaId, datasetId: providedDataset, eventName, value, currency, ctwaClid } = parsed.data;
  ...
  const limiter = rateLimit(`meta-events:send:${workspaceId}:${userData.user.id}`, {
    windowMs: 60_000,
    max: 10,
  });
  if (!limiter.ok) {
  ...
  try {
    ...
    try {
      ...
      logger.error("[meta-events] capi log insert failed", {
        workspaceId,
    ...
    return withCookies(
      NextResponse.json({ ok: true, datasetId, fbtrace_id: fbtraceId })
    );
  } catch (err) {
