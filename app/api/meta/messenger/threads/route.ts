...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      query = query.or(
        `participant_name.ilike.%${search}%,last_message_preview.ilike.%${search}%`
      );
    }
    ...
    const threadsWithTags = threads?.map((thread) => ({
      ...thread,
      tags: thread.messenger_thread_tags?.map((t: { tag: string }) => t.tag) || [],
      messenger_thread_tags: undefined,
    }));
