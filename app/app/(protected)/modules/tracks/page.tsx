...
export default async function TracksModulePage() {
  ...
  const allowed = canAccess({ plan_id: plan.plan_id, is_admin: isAdmin }, "tracks");

  if (!allowed) {
    return (
      <LockedScreen
        title="Tracks is locked"
        description="Upgrade to unlock Tracks orchestration and automation."
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-sm text-white/70">
        Tracks runs consume tokens when executed.
      </div>
    </div>
  );
}
