...
export function CRMInsightsClient({
  ...
  const fetchInsights = useCallback(async () => {
    try {
      ...
      logger.error("[CRM] Failed to fetch insights:", error);
    }
  }, [workspaceId]);
  ...
  // Initial load
  useEffect(() => {
    logger.info(`[CRM] Initialized for workspace: ${workspaceSlug} (${workspaceId})`);
    fetchInsights();
  ...
  const generateInsights = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[CRM] Failed to generate insights:", error);
