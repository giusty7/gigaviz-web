...
export function LeadsManagerClient({
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[Leads] Failed to fetch:", error);
    } finally {
      setLoading(false);
    }
  }, [workspaceId]);
  ...
  // Initial load
  useEffect(() => {
    logger.info(`[Leads] Initialized for workspace: ${workspaceSlug} (${workspaceId})`);
  ...
  const runAIAnalysis = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[Leads] AI analysis failed:", error);
    } finally {
      setAnalyzing(false);
    }
  }, [leads, workspaceId]);
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Select value={filterTemperature} onValueChange={setFilterTemperature}>
                ...
                <SelectContent className="bg-[#0a1229] border-[#d4af37]/20">
                  ...
                  <SelectItem value="warm">ðŸŒ¡ï¸ Warm</SelectItem>
