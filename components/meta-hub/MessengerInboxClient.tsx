...
export function MessengerInboxClient() {
  ...
  async function loadThreads() {
    try {
      setLoading(true);
      let url = `/api/meta/messenger/threads?workspace_id=${workspace?.id}`;
  ...
  async function loadMessages(threadId: string) {
    try {
      const res = await fetch(
        `/api/meta/messenger/threads/${threadId}/messages`
      );
  ...
  async function sendMessage() {
    ...
    try {
      const res = await fetch(
        `/api/meta/messenger/threads/${selectedThread.id}/send`,
        {
          method: 'POST',
  ...
  async function updateThreadStatus(threadId: string, status: string) {
    try {
      const res = await fetch(`/api/meta/messenger/threads/${threadId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
          ...
          <div className="p-4 border-b flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Avatar>
                ...
                <AvatarFallback>
                  {selectedThread.participant_name?.[0]?.toUpperCase() || 'U'}
                </AvatarFallback>
              </Avatar>
              <div>
