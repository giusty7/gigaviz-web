...
export function ImageActions({ imageId, workspaceSlug, title: initTitle, description: initDesc }: Props) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/studio/graph/images/${imageId}`, {
        method: "PATCH",
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/studio/graph/images/${imageId}`, { method: "DELETE" });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
      ...
      router.push(`/${workspaceSlug}/modules/studio/graph/images`);
    } catch {
      setError(t("common.networkErrorShort"));
      setDeleting(false);
    }
      ...
      {editing ? (
        <div className="space-y-3 rounded-xl border border-purple-500/20 bg-[#0a1229]/40 p-4">
          <input
            ...
            placeholder={t("images.actions.titlePlaceholder")}
          />
          <textarea
            value={description}
