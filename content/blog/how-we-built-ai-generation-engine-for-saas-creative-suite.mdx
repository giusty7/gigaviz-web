...
We rejected the obvious approach of adding `/generate` sub-routes to each CRUD API. Instead, we built a single `POST /api/studio/generate` endpoint that handles all 6 types through a Zod discriminated union:

```typescript
const generateSchema = z.discriminatedUnion("type", [
  z.object({ type: z.literal("document"), entityId: z.string().uuid(), prompt: z.string().min(1), category: z.string().optional(), title: z.string().optional() }),
  z.object({ type: z.literal("image"), entityId: z.string().uuid(), prompt: z.string().min(1), style: z.string().optional() }),
  z.object({ type: z.literal("chart"), entityId: z.string().uuid(), prompt: z.string().min(1), chart_type: z.string().optional() }),
  z.object({ type: z.literal("video"), entityId: z.string().uuid(), prompt: z.string().min(1), style: z.string().optional(), duration_seconds: z.number().optional() }),
  z.object({ type: z.literal("music"), entityId: z.string().uuid(), prompt: z.string().min(1), genre: z.string().optional(), bpm: z.number().optional() }),
  z.object({ type: z.literal("dashboard"), entityId: z.string().uuid(), prompt: z.string().min(1), title: z.string().optional() }),
]);
```
...
1. **Validate** → Zod discriminated union parses the request body
2. **Authorize** → Check user session, workspace membership, and entitlement (`graph`, `office`, or `tracks`)
3. **Deduct tokens** → Call `consumeTokens()` RPC before doing any AI work (fail fast on insufficient balance)
...
Tokens are deducted **before** the AI call via a PostgreSQL RPC (`consumeTokens`). This is atomic — if two requests race, only one succeeds. The 402 response triggers a clear "Insufficient tokens" message in the UI.
...
So after the create form submits successfully, if a prompt was provided, we fire a generation request in the background:

```typescript
if (res.ok) {
  const { data } = await res.json();
  
  // Auto-trigger AI generation (fire-and-forget)
  if (prompt.trim()) {
    fetch("/api/studio/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "image", entityId: data.id, prompt: prompt.trim(), style }),
...
The unified generation engine serves 6 content types through 1 endpoint, 1 token system, and 1 status machine. It took approximately 2,800 lines of code across 8 new files:

| Component | Lines | Purpose |
|-----------|-------|---------|
| `lib/studio/ai-generate.ts` | ~350 | 6 AI generation functions |
| `app/api/studio/generate/route.ts` | ~280 | Unified generation endpoint |
| `components/studio/ChartRenderer.tsx` | ~300 | recharts 6-type chart wrapper |
| `components/studio/DashboardRenderer.tsx` | ~170 | Widget grid renderer |
| `components/studio/WaveformVisualizer.tsx` | ~150 | Canvas waveform animation |
| `components/studio/VideoStoryboard.tsx` | ~150 | Scene timeline + script |
| `components/studio/GenerateButton.tsx` | ~130 | Reusable AI trigger |
| `components/studio/UseTemplateButton.tsx` | ~70 | Template cloning |

All passing: typecheck, lint, and build (245 pages generated).
